...
ENV_NAME = '2048'  # Environment name
FRAME_WIDTH = 4  # Resized frame width
FRAME_HEIGHT = 4  # Resized frame height
...
class Agent():
    ...
    def build_network(self):
        model = Sequential()
        model.add(Convolution2D(32, (3, 3), activation='relu', input_shape=(FRAME_WIDTH, FRAME_HEIGHT, STATE_LENGTH)))
    ...
    def get_action(self, state):
        state = np.reshape(state, (FRAME_WIDTH, FRAME_HEIGHT, 1))
...
def main():
    env = Game2048(4)
    agent = Agent(num_actions=env.action_space)
    ...
    if TRAIN:  # Train mode
        for _ in range(NUM_EPISODES):
            ...
            while not terminal:
                ...
                action = agent.get_action(last_observation)
                observation, reward, terminal, _ = env.step(action)
                agent.run(last_observation, action, reward, terminal, observation)
    else:  # Test mode
        # env2048.monitor.start(ENV_NAME + '-test')
        for _ in range(NUM_EPISODES_AT_TEST):
            terminal = False
            observation = env.reset()
            while not terminal:
                last_observation = observation
                action = agent.get_action_at_test(last_observation)
                observation, _, terminal, _ = env.step(action)
                env.render()
        # env2048.monitor.close()
